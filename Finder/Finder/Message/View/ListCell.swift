...
open class ListCell<T>: UICollectionViewCell {
    ...
    public let seperatorView = UIView(backgroundColor: UIColor(white: 0.65, alpha: 0.5))
    func addSeperator(leftSpacing: CGFloat = 0) {
        ...
        seperatorView.anchor(top: nil, bottom: bottomAnchor, leading: leadingAnchor, trailing: trailingAnchor,
                             padding: .init(top: 0, left: leftSpacing, bottom: 0, right: 0), size: .init(width: 0, height: 1))
    }
    func addSeperator(leadingAnchor: NSLayoutXAxisAnchor) {
        addSubview(seperatorView)
        seperatorView.anchor(top: nil, bottom: bottomAnchor, leading: leadingAnchor, trailing: trailingAnchor, size: .init(width: 0, height: 1))
